...
W4 = function(node) if not node then return nil end  return W(node)/8;end
WBig = function(node) if not node then return nil end  return W(node)*1.2;end
...
H4 = function(node) if not node then return nil end  return H(node)/8;end
HBig = function(node) if not node then return nil end  return H(node)*1.2;end
...
function cc.Node:hitTest_Half(point)
    local nsp = self:convertToNodeSpace(point)
    local bb = cc.rect(0,0,W4(self),H4(self))
    if (cc.rectContainsPoint(bb,nsp)) then
        return true
    end
    return false
...
function cc.Node:hitTest_Big(point)
    local nsp = self:convertToNodeSpace(point)
    local bb = cc.rect(0,0,WBig(self),HBig(self))
    if (cc.rectContainsPoint(bb,nsp)) then
        return true
    end
    return false
