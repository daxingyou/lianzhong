...
function UIDragItem:setDragObj(obj)
    ...
    elseif self.dragObj and (not tolua.isnull(self.dragObj)) then
...
function UIDrag:onTouchMoved(point)
    ...
    if  self._isOnBean then
        ...
        if not tolua.isnull(self._currentDragObj) then
        ...
        if self.isISOModel then
            --模拟镜像
            self._currentDragObj:setOpacity(120)
            -- self._currentDragObj:setOpacity(0) -- me 改变的值  透明度为0，全透明，接近原图
            if self._currentDragItem.dragObjFun then
                -- self._currentDragItem:setDragObj(self._currentDragItem:cloneDragObj()) -- me 他才是真正实现克隆对象的操作
            end
        end
...
function UIDrag:onTouchEnded(point)
    ...
    repeat
            ...
            --dump(self._dragItems, "--是啥玩意--")
            if self._dragItems ~= nil then -- me增加的处理，有报错
                
            local target = self._dragItems[i]
            local box = target.dragBox

            --如果没有离开当前物品所在框，则视为不做任何操作和回调
            if self._currentDragItem.dragBox:hitTest_Half(point) then
...
--                self._currentDragItem = nil
                break
            --拖拽离开盒子
            elseif  box:hitTest_Big(point) then
                ...
                if not target.dragObj then
                    ...
                    if self.isExchangeModel then
                        ...
                        self._currentDragObj:release()
                        --当前框置放目标框的对象
                        -- self._currentDragItem:setDragObj(tempTagObj) -- me 直接单击牌面，会报错，提示多次添加这个对象
                        tempTagObj:release()
    ...
    if not result then
        ...
        if self._currentDragObj~=nil and not tolua.isnull(self._currentDragObj) then
            self._currentDragObj:retain()
            self._currentDragObj:removeFromParent()
            self._currentDragItem:setDragObj(nil)
            self._currentDragObj:setOpacity(255)
    ...
    --    self._isBean = false
    self._currentDragItem = nil
    -- self:setTouchSwallowEnabled(false) -- me屏蔽的，有报错
end