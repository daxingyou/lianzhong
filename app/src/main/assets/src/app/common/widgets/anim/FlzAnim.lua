...
-- 神一样的动画。
-- 类申明
local FlzAnim = class("FlzAnim",
	function()
		 return display.newNode()
	end
)

function FlzAnim:ctor(seatNo)
    ...
    self:create(cardNumAll, seatNo)
    --end, 1)
end

function FlzAnim:create(cardNum, seatNo)
	...
	display.newSprite(Imgs.gameing_out_flz)
		 ...
		 for j = 1, 3 do 
			...
			if seatNo==nil then
					if j == 3 then
					 	...
					 	-- transition.moveTo(poker, {time = .1, x = SELF_POS.x, y = SELF_POS.y-otherPokerMove, delay = 0.05 * delayTime/2})
					 	local a = transition.moveTo(poker, {time = .1, x = SELF_POS.x, y = SELF_POS.y-otherPokerMove, delay = 0.15 * delayTime/2})
					...
					if j == 2 then
					 	...
					 	transition.moveTo(poker, {time = .1,x = RIGHT_POS.x , y = RIGHT_POS.y-otherPokerMove, delay = 0.15 * delayTime/2})
					end

					if j == 1 then
						table.insert(LEFT_CARDS,poker)
						transition.moveTo(poker, {time = .1,x = LEFT_POS.x , y = LEFT_POS.y-otherPokerMove, delay = 0.15 * delayTime/2})
					end
			else
				if seatNo == CEnum.seatNo.me then
					if j == 3 then
					 	...
					 	-- transition.moveTo(poker, {time = .1, x = SELF_POS.x, y = SELF_POS.y-otherPokerMove, delay = 0.05 * delayTime/2})
					 	local a = transition.moveTo(poker, {time = .1, x = SELF_POS.x, y = SELF_POS.y-otherPokerMove, delay = 0.15 * delayTime/2})
					 	local b = transition.fadeTo(poker, {opacity = 1, time = .1, delay = 0.65 * delayTime/2, onComplete=function() self:removeFromParent() end})
					 	cc.Sequence:create(a,b)
					end
				end

				if seatNo == CEnum.seatNo.R then
					if j == 2 then
					 	table.insert(RIGHT_CARDS,poker)
					 	local a = transition.moveTo(poker, {time = .1,x = RIGHT_POS.x , y = RIGHT_POS.y-otherPokerMove, delay = 0.15 * delayTime/2})
					 	local b = transition.fadeTo(poker, {opacity = 1, time = .1, delay = 0.65 * delayTime/2, onComplete=function() self:removeFromParent() end})
					 	cc.Sequence:create(a,b)
					end
				end

				if seatNo == CEnum.seatNo.L then
					if j == 1 then
						table.insert(LEFT_CARDS,poker)
						local a = transition.moveTo(poker, {time = .1,x = LEFT_POS.x , y = LEFT_POS.y-otherPokerMove, delay = 0.15 * delayTime/2})
						local b = transition.fadeTo(poker, {opacity = 1, time = .1, delay = 0.65 * delayTime/2, onComplete=function() self:removeFromParent() end})
						cc.Sequence:create(a,b)
					end
				end
