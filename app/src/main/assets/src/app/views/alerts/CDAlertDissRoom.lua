...
-- 类申明
local CDAlertDissRoom = class("CDAlertDissRoom",
	function()
...
function CDAlertDissRoom:ctor(res_data)
    ...
    if CVar._static.NavBarH_Android ~= CEnum.status.NavBarH_def then
    ...
    cc.ui.UIImage.new(Imgs.dialog_content_bg,{scale9=false})
        :addTo(self.pop_window)
        -- :align(display.CENTER, display.cx, osHeight -gaping_h-gaping_y -(osHeight-gaping_h*2-gaping_y)/2 )
...
function CDAlertDissRoom:create_content(res_data)
	...
	if res_data ~= nil then
		...
		if Commons:checkIsNull_str(descript) then
			if dissRoom_title and not tolua.isnull(dissRoom_title) then
	    ...
	    if Commons:checkIsNull_tableList(playerChooses) then
	    	if dissRoom_players_node and not tolua.isnull(dissRoom_players_node) then
	    ...
	    if Commons:checkIsNull_number(overtime) then
	    	if dissRoom_second and not tolua.isnull(dissRoom_second) then
	        ...
	        local function CDAlertDissRoom_changeSecond()
	        	overtime = overtime - 1
		    	dissRoom_second:setString(""..overtime)
		    	if overtime <= 0 then
		    		if self.top_scheduID_dissRoom ~= nil then
			        	self.top_scheduler:unscheduleScriptEntry(self.top_scheduID_dissRoom)
			        	self.top_scheduID_dissRoom = nil
			        end
		    	end
	        end
	        if self.top_scheduler~=nil then
		        ...
		        self.top_scheduID_dissRoom = self.top_scheduler:scheduleScriptFunc(CDAlertDissRoom_changeSecond, 1, false) -- 1秒一次
		    end
	    end
	    ...
	    -- 	dissRoom_cancelBtn, dissRoom_submitBtn, self.top_scheduID_dissRoom
    end
end

function CDAlertDissRoom:onExit()
	self:dispose()
end

function CDAlertDissRoom:myExit()
	self:dispose()
end

function CDAlertDissRoom:dispose()
	if self.top_scheduID_dissRoom ~= nil then
    	self.top_scheduler:unscheduleScriptEntry(self.top_scheduID_dissRoom)
    	self.top_scheduID_dissRoom = nil
